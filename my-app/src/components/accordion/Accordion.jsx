...
const Accordion = () => {
    ...
    return (
        <div className={"wrapper"}>
            <button onClick={() => setEnableMultiSelection(!enableMultitSelection)}>Enable Multi Selection</button>
                    ...
                    data.map((item) => (
                        <div className={"wrapper_accordion-wrapper_item"}>
                            <div onClick={
                                ...
                                <h3>{item.question}</h3>
                                {selected === item.id || multiple.indexOf(item.id) !== -1 ? <span>-</span> : <span>+</span>}
                            </div>
                            {
                                enableMultitSelection ?
